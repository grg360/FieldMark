...
## Pagination / scrolling ✅ SHIPPED (May 3 Sunday evening)

**Status:** Pagination shipped to product May 3 Sunday evening. Implementation differs slightly from initial scope — uses "Load more" button rather than infinite scroll on scroll-end, which proved more controlled and easier to debug. "Showing X of Y" counter implemented in section header. "Jump to top" button NOT implemented (not needed when user controls pagination via button rather than scrolling deep automatically).

**What shipped:**
- Backend: `getRisingStars` accepts `options.offset`, returns `{ rows, total }` shape. Uses `.range(offset, offset + limit - 1)` instead of hardcoded `.limit(200)`. Separate count query precedes row query.
- Frontend (App.tsx): `feedOffset`, `feedTotal`, `loadingMore` state. `loadMore` function appends pages. Section header shows "X of Y identified" when partial, total only when fully loaded. useEffect resets offset on TA/darkHorse change. Manual refresh resets to first page. Green "Load more" button at bottom of card list.

**Critical product finding from pagination ship:** Past row 20-30, the publication velocity formula plateau becomes visible — every HCP card past that depth shows identical scoring values (composite=47.3, pub_vel=62.7x, cit_traj=+0.0%, trials=—). This is Foundation Issue 3 made visible in product. Pub velocity formula redesign elevated to active P0 (priority doc 8b).

**v1.5+ enhancement (deferred):**
- Optional infinite scroll on scroll-end as alternative to button (current button is fine for the "I'm exploring" use case; scroll-end might be better for the "I'm hunting" use case)
- "Jump to top" button after deep scroll
- "Load all" / "Show top 100" affordance for power users

Original scope notes from May 2:
- ~~Once filters are applied, the cohort might still be hundreds of HCPs.~~
- Server-side pagination via Supabase `range()` or offset/limit. ~~Existing query in `getRisingStars` already uses `.limit(200)` which is hard-capped — needs to become parameterized.~~ Done.
