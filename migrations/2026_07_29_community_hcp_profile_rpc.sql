-- Community HCP profile (direction 1a "two spines") — stage 1. Read-only SECURITY
-- DEFINER aggregation of the PUBLIC/derived sections. Per-user Field Insights, follow-ups
-- and notes are fetched by the authenticated client under RLS (not here).
--
-- The two profiles share one route (/hcp/:id/brief) and dispatch on the PUBLICATION
-- record: hcp_profile_spine() returns 'academic' when the HCP has ≥1 sourced position,
-- else 'community' — keying the route on the thing that actually determines which spine
-- can render (a belief profile cannot render for someone with no publications).
--
-- Source of fact per section (reported):
--   practice shape  → hcp_community_ranks_v2 (patient_volume, setting, career) + drug
--                     breadth = distinct drugs in hcp_open_payments_by_drug_v2
--   community score → hcp_community_ranks_v2 (composite_score, rank, scope_size) + the
--                     40/30/15/10/5 weights (constants) + publication_signal (real 0)
--   engagement record (PRIMARY) → hcp_open_payments_by_drug_v2 (per-product transfers,
--                     counts, most_recent_date, year_over_year_trend_pct, yearly totals)
--   engagement mix / entities / timeline → hcp_open_payments_summary_v2 +
--                     hcp_open_payments_top_companies_v2
--   narrative (why-this / signal / why-now / angle / caution) → hcp_narratives_v2
--
-- Composed frame elements with NO backing (render honest state, reported): per-component
-- score CONTRIBUTION numbers (not persisted — only composite + raw signals are), the
-- "4 sites" practice count, "43.1K claims/5yr" exact figure, pre-2022 timeline years,
-- multi-year sparklines (only 3 yearly columns exist), field-intelligence chips.
...
begin
  ...
  entities as (
    ...
    from hcp_open_payments_top_companies_v2 where hcp_id = p_hcp_id order by rank_by_amount limit 8
  ),
  narr as (
  ...
  select json_build_object(
    ...
    'entities', (select json_agg(json_build_object('name', nm, 'amount', amt, 'payments', cnt)) from entities),
