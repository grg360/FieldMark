-- rising_ledger: expand the EUROPE sentinel in p_countries.
-- Date: 2026-08-18. Branch: resurfacing.
-- Revert: sql/revert/2026_08_18_established_europe_scope_REVERT.sql (section 2 --
--         it already carries this function's pre-change body, captured this morning
--         and re-verified byte-for-byte against the live definition before this
--         migration was written).
--
-- WHAT AND WHY. The ledger's Europe selection used to send the 33 country codes
-- from lib/cohortLedger.ts. It now sends the single sentinel 'EUROPE', so the
-- country list lives ONLY in region_countries and the frontend, this function,
-- rising_board and recompute_established_ranks_v3.py cannot drift apart.
--
-- The sentinel is expanded in the base predicate; scope_label learns EUROPE as a
-- pool name alongside GLOBAL. Rows keep their own scored_country, so the location
-- chip is unaffected.
--
-- ORDER OF DEPLOY: apply this BEFORE shipping the frontend change. The old frontend
-- (33 explicit codes) works against this version unchanged -- the country list is
-- still honoured. The new frontend against the OLD function matches no country and
-- returns an EMPTY LEDGER, silently. So: RPC first, frontend second.
--
...
AS $function$
  ...
  sel as (select ('__global__' = any(p_countries)) as is_global,
                 ('EUROPE' = any(p_countries)) as is_europe),
  ...
  base as (
    ...
    where (sel.is_global
           or (sel.is_europe and rk.eff in (select country_code from europe))
      ...
      select coalesce(json_agg(row_to_json(t) order by t.rank), '[]'::json) from (
        select top.rank, top.global_rank, top.country_rank, top.europe_rank,
               ...
               case when (select is_global from sel) then 'GLOBAL'
                    when (select is_europe from sel) then 'EUROPE'
