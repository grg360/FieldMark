-- hcp_profile_brief: resolve the score row US-else-global, and carry the country.
-- Date: 2026-08-19. Branch: resurfacing.
-- Revert: sql/revert/2026_08_19_hcp_profile_brief_scope_REVERT.sql
--
-- TWO DEFECTS, ONE FUNCTION.
--
-- 1. SCORES BLANK FOR EVERY NON-US HCP. The `est` CTE was pinned to region/US, so an
--    HCP with no US row got NULL for all four header cells. Martin Reck is global #3
--    with cohort_score 99.968, sci 99.98, net 99.95 -- his profile rendered four
--    em-dashes. The global rank was already read separately, so the profile knew he
--    was #3 while showing him as unscored.
--
-- 2. NO COUNTRY ON THE PAYLOAD, so the US-only sections (Medicare Part B, Open
--    Payments, NIH RePORTER) had nothing to gate on and rendered their absence states
--    for non-US records -- a German oncologist reading "Medicare Part B cannot be read
--    for this record".
--
-- The scope the score came from is returned as scope_label, following the ledger's
-- convention rather than inventing a second one. Callers must render it: an unlabelled
-- rank that silently switches pools is worse than the blank it replaces.
--
