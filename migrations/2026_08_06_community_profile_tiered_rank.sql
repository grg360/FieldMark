-- community_hcp_profile: profile rank reads the ledger's tiered ordering (2026-08-06)
-- Fixes the same class as Tuesday's hcp_community_ranks_v2/community_ledger split:
-- the profile showed the raw normalized_score rank (Uyeki 62) while the ledger
-- showed the tiered rank (26). Now both use community_ledger's ORDER BY over the
-- full US board, so one person cannot carry two numbers on adjacent surfaces.
...
begin
  ...
  -- Tiered rank (2026-08-06): the profile must show the SAME position as the
  -- community ledger, not hcp_community_ranks_v2's raw normalized_score rank —
  -- two orderings for one person on adjacent surfaces read as a bug (Uyeki 26 on
  -- the ledger, 62 here). Compute the rank with community_ledger's EXACT ORDER BY
  -- over the full US community board (all tiers, so candidate/unresolved HCPs get
  -- a coherent position too), mirroring community_tiered_ranks. scope_size stays
  -- the full-board count from crank, so the denominator is unchanged.
  tiered as (
    select tiered_rank from (
      select c.hcp_id,
             row_number() over (
               order by
      ...
      join hcps_v2 h2 on h2.id = c.hcp_id
      join hcp_nsclc_evidence_tier_v1 e on e.hcp_id = c.hcp_id
      where c.therapeutic_area_id = v_ta and h2.country = 'US'
    ) r where r.hcp_id = p_hcp_id
  ),
  sigs as (
  ...
  select json_build_object(
    ...
    'score', (select json_build_object(
        ...
        'rank', coalesce((select tiered_rank from tiered), rank), 'scope_size', scope_size,
