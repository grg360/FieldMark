-- established_ledger: carry the two displayed quantities.
-- Date: 2026-08-18. Branch: resurfacing.
-- Revert: sql/revert/2026_08_18_established_ledger_quantities_REVERT.sql
--
-- WHY. The ledger's two ranking columns (SCI CEILING / NET CEILING) are percentiles
-- that saturate: every row in the visible head printed 99.9. Measured on the Europe
-- board, ranks 1-5 are 99.976 / 99.952 / 99.938 / 99.910 / 99.892 and all five
-- rendered as the same number. The columns were carrying no information where a
-- reader most needs it.
--
-- Both inputs are percentile-of-percentile besides: network_influence_score is
-- itself a blend of three centrality percentiles (0-100, mean 49.97) which the
-- scorer then re-ranks within scope.
--
-- THIS MIGRATION IS ADDITIVE ONLY. Two new output fields, two LEFT JOINs, nothing
-- removed or renamed -- the old frontend keeps working against it unchanged, so the
-- deploy order is free. The column swap itself is frontend-side.
--
-- NOT IN THIS CHANGE, deliberately: the composite still mixes reference populations
-- (scientific is TA-wide across 17,011, network is re-percentiled per scope, so the
-- same person scores 99.988 in DE / 99.976 in EUROPE / 99.972 global). That is a
-- Tier-2 board change with its own capture and revert, shipping after this one so
-- the two are distinguishable in the diff and on screen.
--
