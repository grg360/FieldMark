-- TA name repoint: resolve the NSCLC therapeutic area by SLUG, not by NAME.
-- Date: 2026-08-15
--
-- WHY, AND WHY THIS SHIPS BEFORE THE RENAME.
-- therapeutic_areas.name is about to change from 'NSCLC' to 'Lung Cancer'. Ten
-- live functions resolved the TA by that literal name. On rename they would not
-- have errored -- they would have resolved to NULL and returned EMPTY RESULTS,
-- silently, across both cohort ledgers and every profile RPC.
--
-- The slug ('nsclc') is explicitly NOT changing and is the stable key. Routes,
-- config/therapeutic_areas/nsclc.json and hcp_narratives_v2.therapeutic_area_slug
-- are all untouched by the rename.
--
-- TWO FORMS were in the wild, which is why a find-replace on one pattern would
-- have missed a function:
--   9 x  select id from therapeutic_areas where name = 'NSCLC'   (subselect)
--   1 x  join therapeutic_areas ta on ta.id = ... and ta.name = 'NSCLC'
--        (hcp_profile_spine -- a JOIN PREDICATE, not a subselect)
--
-- Four of the ten (community_hcp_profile, established_ledger, hcp_profile_brief,
-- rising_ledger) ALREADY carried slug='nsclc' predicates elsewhere in their
-- bodies, so they were resolving the same TA two different ways. After this they
-- resolve one way.
--
-- Bodies below are the LIVE definitions from pg_get_functiondef with ONLY the
-- resolution predicate changed -- verified line-by-line, so this migration
-- cannot carry an unrelated behaviour change.
--
-- REVERT: sql/revert/2026_08_15_ta_name_repoint_REVERT.sql
-- NOT TOUCHED: the 32 historical migrations carrying the same pattern.

BEGIN;

-- == asset_authors(text,integer)  (1 subselect, 0 join-predicate) ==
  ...
  ranked as (
    ...
    left join lateral (
      ...
      where r.hcp_id = a.hcp_id
        and r.therapeutic_area_id = (select id from therapeutic_areas where slug = 'nsclc')
      order by rank asc
      limit 1
    ) r on true
  )
...
-- == asset_overview(text)  (1 subselect, 0 join-predicate) ==
...
-- == community_hcp_profile(uuid)  (1 subselect, 0 join-predicate) ==
...
-- == community_practice_profile(uuid)  (1 subselect, 0 join-predicate) ==
...
-- == established_ledger(integer,integer,text[],text[])  (1 subselect, 0 join-predicate) ==
...
-- == hcp_administered_therapy(uuid)  (1 subselect, 0 join-predicate) ==
  ...
  WHERE d.hcp_id = p_hcp_id
    ...
    AND EXISTS (
      ...
      WHERE t.hcpcs_code = d.hcpcs_code
        ...
        AND t.therapeutic_area_id = (SELECT id FROM therapeutic_areas WHERE slug = 'nsclc')
    );
$function$;

-- == hcp_profile_brief(uuid)  (1 subselect, 0 join-predicate) ==
...
-- == hcp_profile_spine(uuid)  (0 subselect, 1 join-predicate) ==
...
AS $function$
  select case when exists (
    ...
    join therapeutic_areas ta on ta.id = e.therapeutic_area_id and ta.slug = 'nsclc'
    where e.hcp_id = p_hcp_id
      and e.scope_type = 'global'
  ) then 'academic' else 'community' end;
$function$;

-- == ledger_meta(text)  (1 subselect, 0 join-predicate) ==
...
-- == rising_ledger(integer,integer,text[],text[])  (1 subselect, 0 join-predicate) ==
...
-- Assertion: no live function may still resolve this TA by name.
DO $$
DECLARE v_bad text;
BEGIN
  ...
  WHERE n.nspname = 'public' AND p.prokind = 'f'
    AND pg_get_functiondef(p.oid) ~* 'name[[:space:]]*=[[:space:]]*''NSCLC''';
  IF v_bad IS NOT NULL THEN
    RAISE EXCEPTION 'still resolving the TA by name: %', v_bad;
  END IF;
  RAISE NOTICE 'repoint complete -- no live function resolves this TA by name';
END $$;
