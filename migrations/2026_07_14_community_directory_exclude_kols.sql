/* get_community_directory_filtered + _count — REVISION: exclude designated KOLs.
   Supersedes migrations/2026_07_14_get_community_directory_filtered.sql (fresh
   CREATE OR REPLACE; the base migration is left untouched on disk).

   POLICY: Community is the population NOT already surfaced as designated KOLs.
   - EXCLUDE any practitioner whose matched_hcp_id is in the AD Established
     (hcp_established_ranks_v3) OR AD Rising (hcp_rising_composite_v1) cohort.
     195 of 342 matched rows -> new directory total ~19,156.
   - is_published_kol = (matched_hcp_id IS NOT NULL). POST-exclusion this is only
     the ~147 "discovery" cases (matched to the corpus but NOT designated KOLs).

   Anti-join uses NOT EXISTS (not a JOIN) against the two AD cohort tables — those
   have multiple scope rows per hcp_id, so a JOIN would fan out (the earlier
   537-vs-342 discrepancy). NOT EXISTS short-circuits and never multiplies.
   AD ta_id = 9e4139d2-e062-4a58-8728-cdabb2d7dca1.

   Return-type change (new is_published_kol column) requires DROP+CREATE for the
   directory function; the _count function is body-only. Run as standalone
   statements (no BEGIN/COMMIT). */

-- =============================================================================
-- 1. Directory rows — DROP first (return type gains is_published_kol)
-- =============================================================================

DROP FUNCTION IF EXISTS public.get_community_directory_filtered(text, text, boolean, text, text, integer, integer);

CREATE FUNCTION public.get_community_directory_filtered(
  p_state text,
  p_taxonomy_label text,
  p_ad_only boolean,
  p_search text,
  p_sort text,
  p_limit integer,
  p_offset integer
)
RETURNS TABLE(
  npi_number text,
  first_name text,
  last_name text,
  credentials text,
  practice_city text,
  practice_state text,
  primary_taxonomy_label text,
  is_sole_proprietor boolean,
  career_stage_years integer,
  matched_hcp_id uuid,
  is_published_kol boolean,
  total_payments_3yr numeric,
  ad_drug_payments_3yr numeric,
  top_manufacturers jsonb,
  top_drugs jsonb
)
LANGUAGE sql
STABLE
AS $function$
  SELECT
    cp.npi_number,
    cp.first_name,
    cp.last_name,
    cp.credentials,
    cp.practice_city,
    cp.practice_state,
    cp.primary_taxonomy_label,
    cp.is_sole_proprietor,
    cp.career_stage_years,
    cp.matched_hcp_id,
    (cp.matched_hcp_id IS NOT NULL) AS is_published_kol,
    pay.total_payments_3yr,
    pay.ad_drug_payments_3yr,
    pay.top_manufacturers,
    pay.top_drugs
  FROM community_practitioners cp
  LEFT JOIN community_practitioner_payments pay
    ON pay.npi_number = cp.npi_number
  WHERE (p_state IS NULL OR p_state = 'All' OR cp.practice_state = p_state)
    AND (p_taxonomy_label IS NULL OR p_taxonomy_label = 'All'
         OR cp.primary_taxonomy_label = p_taxonomy_label)
    AND (NOT p_ad_only OR pay.ad_drug_payments_3yr > 0)
    AND (
      p_search IS NULL OR p_search = ''
      OR (cp.first_name || ' ' || cp.last_name) ILIKE '%' || p_search || '%'
      OR cp.practice_city ILIKE '%' || p_search || '%'
    )
    -- Exclude designated KOLs (already surfaced in AD Established/Rising).
    AND NOT EXISTS (
      SELECT 1 FROM hcp_established_ranks_v3 er
      WHERE er.hcp_id = cp.matched_hcp_id
        AND er.therapeutic_area_id = '9e4139d2-e062-4a58-8728-cdabb2d7dca1'
    )
    AND NOT EXISTS (
      SELECT 1 FROM hcp_rising_composite_v1 rc
      WHERE rc.hcp_id = cp.matched_hcp_id
        AND rc.therapeutic_area_id = '9e4139d2-e062-4a58-8728-cdabb2d7dca1'
    )
  ORDER BY
    CASE WHEN p_sort = 'ad'     THEN pay.ad_drug_payments_3yr END DESC NULLS LAST,
    CASE WHEN p_sort = 'total'  THEN pay.total_payments_3yr   END DESC NULLS LAST,
    CASE WHEN p_sort = 'tenure' THEN cp.career_stage_years    END DESC NULLS LAST,
    CASE WHEN p_sort = 'name'   THEN cp.last_name  END ASC,
    CASE WHEN p_sort = 'name'   THEN cp.first_name END ASC,
    cp.last_name ASC, cp.first_name ASC, cp.npi_number ASC   -- deterministic paging tiebreak
  LIMIT p_limit OFFSET p_offset;
$function$;

-- =============================================================================
-- 2. Result count — same filters + same exclusion (body-only replace)
-- =============================================================================

CREATE OR REPLACE FUNCTION public.get_community_directory_filtered_count(
  p_state text,
  p_taxonomy_label text,
  p_ad_only boolean,
  p_search text
)
RETURNS integer
LANGUAGE sql
STABLE
AS $function$
  SELECT COUNT(*)::int
  FROM community_practitioners cp
  LEFT JOIN community_practitioner_payments pay
    ON pay.npi_number = cp.npi_number
  WHERE (p_state IS NULL OR p_state = 'All' OR cp.practice_state = p_state)
    AND (p_taxonomy_label IS NULL OR p_taxonomy_label = 'All'
         OR cp.primary_taxonomy_label = p_taxonomy_label)
    AND (NOT p_ad_only OR pay.ad_drug_payments_3yr > 0)
    AND (
      p_search IS NULL OR p_search = ''
      OR (cp.first_name || ' ' || cp.last_name) ILIKE '%' || p_search || '%'
      OR cp.practice_city ILIKE '%' || p_search || '%'
    )
    AND NOT EXISTS (
      SELECT 1 FROM hcp_established_ranks_v3 er
      WHERE er.hcp_id = cp.matched_hcp_id
        AND er.therapeutic_area_id = '9e4139d2-e062-4a58-8728-cdabb2d7dca1'
    )
    AND NOT EXISTS (
      SELECT 1 FROM hcp_rising_composite_v1 rc
      WHERE rc.hcp_id = cp.matched_hcp_id
        AND rc.therapeutic_area_id = '9e4139d2-e062-4a58-8728-cdabb2d7dca1'
    );
$function$;

-- =============================================================================
-- 3. Grants (DROP cleared the directory grant; re-apply. authenticated only)
-- =============================================================================

GRANT EXECUTE ON FUNCTION public.get_community_directory_filtered(text, text, boolean, text, text, integer, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_community_directory_filtered_count(text, text, boolean, text) TO authenticated;

-- =============================================================================
-- 4. Reload PostgREST schema cache
-- =============================================================================

NOTIFY pgrst, 'reload schema';
