...
// getRisingStars / getEstablished / getCommunity REMOVED 2026-09-24 with the card feed,
// along with fetchCohortViaRpc above, which existed only to serve them. App.tsx was their
// only caller. The ledger reads board_established / board_rising / community_ledger
// instead, and that difference is why the two surfaces could disagree: getRisingStars
// carried an isAdRising branch reading hcp_rising_composite_v1 for Atopic Dermatitis while
// the ledger read hcp_rising_star_ranks_v3 -- two tables sharing zero hcp_ids in every TA.
//
// COMPOSITE IS NOT GONE FROM THE APP. getHCPDetail below still reads
// hcp_rising_composite_v1 for AD (its own isAdRising), and so does the institution rising
// count. Both are live surfaces on a superseded Rising definition; retiring the feed did
// not retire them, and they need a decision rather than a deletion.
