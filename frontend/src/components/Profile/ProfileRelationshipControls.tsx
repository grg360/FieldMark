...
function Bookmark({ on }: { on: boolean }) {
  return (
    <svg width="13" height="16" viewBox="0 0 12 15" aria-hidden>
      <path d="M1 1.5h10v12l-5-3.2-5 3.2z" fill={on ? CANON.INK.PRIME : "none"} stroke={on ? CANON.INK.PRIME : CANON.INK.LABEL} strokeWidth="1.2" strokeLinejoin="round" />
    </svg>
  );
}
...
const btn = {
  ...
  font: `500 11px ${FACE.data}`, letterSpacing: ".08em", color: CANON.INK.BODY,
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        <button
          ...
          style={{ ...btn, color: tracked ? CANON.INK.PRIME : CANON.INK.BODY, borderColor: tracked ? "rgba(255,255,255,.28)" : "rgba(255,255,255,.14)" }}
