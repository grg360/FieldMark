...
import { COLOR, FONT, GROUND, LINE, GOLD, COOL, WARM } from "../../lib/designTokens";
import {
  getModerationQueue,
  getThread,
  type ForumPost,
...
function AttachedNote({ post }: { post: ForumPost }) {
  ...
  const accent = isPeer ? COLOR.amber : COOL.chromeStrong;
  const tint = isPeer ? "rgba(232,160,32,0.05)" : GROUND.g2;
  const border = isPeer ? "rgba(232,160,32,0.2)" : LINE.l2;
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: "12px 14px", background: tint, border: `1px solid ${border}` }}>
      ...
      <span style={{ fontSize: 12.5, lineHeight: 1.6, color: COOL.muted }}>{note.body_text}</span>
      <span style={mono(9.5, COOL.label)}>{note.disposition_text}</span>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10, padding: "16px 18px", background: "rgba(232,112,78,0.05)", border: "1px solid rgba(232,112,78,0.24)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        ...
        <span style={mono(10.5, COOL.chrome)}>{post.author_handle} · posted {post.recency_label}</span>
        <ProvenanceChip seed={post.is_seed} />
      </div>
      <span style={{ fontSize: 13, lineHeight: 1.6, color: COOL.prose }}>
        {record?.clause_text ??
      ...
      <span style={mono(9.5, COOL.label)}>
      ...
      <span style={mono(9.5, COOL.label)}>Reply chain below this post preserved — thread not locked</span>
      ...
      {revealed && (
        <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: "12px 14px", background: GROUND.g1, border: `1px dashed rgba(232,112,78,0.3)` }}>
          <span style={{ ...mono(9, COOL.label), letterSpacing: "0.12em" }}>REMOVED TEXT · VISIBLE TO REVIEWERS AND THE AUTHOR ONLY</span>
          <span style={{ fontFamily: FONT.serif, fontSize: 14.5, lineHeight: 1.65, color: WARM.muted }}>{post.removed_body}</span>
...
function PostByline({ post, isMobile }: { post: ForumPost; isMobile: boolean }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 9, flexWrap: "wrap" }}>
      ...
      <span style={mono(12, COOL.ui)}>{post.author_handle}</span>
      <VerifiedBadge small />
      <span style={mono(10, COOL.label)}>{post.recency_label}</span>
...
function PostActions({ post, isMobile, onReply }: { post: ForumPost; isMobile: boolean; onReply?: (post: ForumPost) => void }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
      ...
      {onReply ? (
        <button type="button" onClick={() => onReply(post)} style={{ ...mono(10.5, COOL.prose), letterSpacing: "0.08em", background: "none", border: `1px solid ${LINE.l2}`, borderRadius: 0, padding: "8px 12px", cursor: "pointer", ...touch }}>Reply</button>
      ) : (
...
function PostBody({ post, record, isMobile, onReply }: { post: ForumPost; record?: ModerationRecord; isMobile: boolean; onReply?: (post: ForumPost) => void }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <p style={{ margin: 0, fontFamily: FONT.serif, fontSize: 15, lineHeight: 1.7, color: WARM.prose, maxWidth: "72ch" }}>
        {post.body}
      </p>
...
export default function ThreadPage() {
  ...
  if (!loaded) {
    return (
      <AppLayout width="reading">
        <div style={{ paddingTop: 24, ...mono(11, COOL.label) }}>Loading…</div>
      </AppLayout>
    );
  }
  if (!detail) {
    return (
      <AppLayout width="reading">
        <div style={{ paddingTop: 24, fontFamily: FONT.sans, color: COOL.prose }}>
          <div style={{ ...mono(10.5, COOL.chrome), letterSpacing: "0.14em", marginBottom: 8 }}>THREAD NOT FOUND</div>
          <Link to="/field-intelligence" style={{ color: GOLD.gold }}>← All threads</Link>
        </div>
      </AppLayout>
    );
  }
  ...
  const renderPost = (post: ForumPost, nested: boolean, parentHandle?: string) => {
    ...
    const indentStyle = nested && !flat
      ? { marginLeft: 34, borderLeft: `1px solid ${LINE.l0}`, paddingLeft: 20 }
      : undefined;
    return (
      <div key={post.id} style={indentStyle}>
        <div style={{ display: "flex", flexDirection: "column", gap: 10, padding: "18px 0", borderBottom: `1px solid ${LINE.l0}` }}>
          {nested && flat && post.compliance_state !== "removed" && parentHandle && (
            <span style={mono(9.5, COOL.label)}>replying to {parentHandle}</span>
          )}
        ...
        {hiddenRemoved.length > 0 && (
          <div style={flat ? undefined : { marginLeft: 34, paddingLeft: 20, borderLeft: `1px solid ${LINE.l0}` }}>
            <div style={{ ...mono(10, COOL.label), padding: "12px 0", fontStyle: "italic" }}>
  ...
  return (
    <AppLayout width="reading">
      <div style={{ fontFamily: FONT.sans, color: COOL.ui, paddingTop: 16, display: "flex", flexDirection: "column", gap: 18 }}>
        <PrototypeStrip />
        <Link to="/field-intelligence" style={{ ...mono(10.5, COOL.muted), letterSpacing: "0.1em" }}>← ALL THREADS</Link>
        ...
        <div style={{ padding: isMobile ? "12px 14px" : 20, background: GROUND.g2, border: `1px solid ${LINE.l1}`, borderLeft: `2px solid ${GOLD.gold}`, display: "flex", flexDirection: "column", gap: isMobile ? 8 : 12, ...(isMobile ? { position: "sticky" as const, top: 0, zIndex: 10 } : {}) }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <span style={{ ...mono(9.5, GOLD.gold), letterSpacing: "0.14em", background: "transparent", border: `1px solid ${GOLD.dim}`, padding: "3px 7px", fontWeight: 600 }}>PUBLICATION ANCHOR</span>
            <span style={mono(11, COOL.muted)}>PMID {anchor.pubmed_id}</span>
            <span style={mono(11, COOL.muted)}>{anchor.journal_abbrev} · {anchor.publication?.pub_year ?? "—"}</span>
            <span style={mono(11, COOL.label)}>{(anchor.publication?.citation_count ?? 0)} citations</span>
          </div>
          <h2 style={{ margin: 0, fontFamily: FONT.serif, fontSize: 22, fontWeight: 500, lineHeight: 1.35, color: COOL.ui }}>
            {anchor.publication?.title ?? "—"}
          </h2>
          <div style={{ display: "flex", alignItems: "center", gap: 18, flexWrap: "wrap" }}>
            <a href={`https://pubmed.ncbi.nlm.nih.gov/${anchor.pubmed_id}/`} target="_blank" rel="noreferrer" style={{ ...mono(11, GOLD.gold) }}>Open in PubMed ↗</a>
            <span style={mono(11, COOL.label)}>Author list not reproduced in-product</span>
          </div>
          ...
          {isMobile ? (
            <span style={mono(9.5, COOL.label)}>Scope: results as published · tap to read the paper</span>
          ) : (
            <div style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "11px 13px", background: GROUND.g1, border: `1px solid ${LINE.l1}` }}>
              <span style={{ ...mono(9.5, GOLD.gold), letterSpacing: "0.12em", flexShrink: 0, paddingTop: 2, fontWeight: 600 }}>SCOPE</span>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            ...
            <span style={mono(12.5, COOL.ui)}>{thread.author_handle}</span>
            <VerifiedBadge />
            <span style={mono(10.5, COOL.label)}>{thread.recency_label}</span>
            <ProvenanceChip seed={thread.is_seed} />
          </div>
          <h1 style={{ margin: 0, fontFamily: FONT.serif, fontSize: 27, fontWeight: 400, lineHeight: 1.35, color: COOL.ui }}>
            {thread.question_title}
          </h1>
          {thread.question_body && (
            <p style={{ margin: 0, fontFamily: FONT.serif, fontSize: 15.5, lineHeight: 1.7, color: WARM.prose, maxWidth: "70ch" }}>
              {thread.question_body}
            </p>
          )}
          <div style={{ display: "flex", gap: 14, alignItems: "center", ...mono(10.5, COOL.label), flexWrap: "wrap" }}>
        ...
        <div style={{ height: 1, background: LINE.l1, margin: "8px 0 0" }} />

        {/* ── Reply tree ── */}
        <div>{roots.map((p) => renderPost(p, false))}</div>

        {/* ── Reply composer — rendered, inert ── */}
        <div style={{ marginTop: 16, padding: 18, background: GROUND.g2, border: `1px solid ${LINE.l1}`, display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <span style={{ ...mono(9, GOLD.gold), letterSpacing: "0.12em", background: "transparent", border: `1px solid ${GOLD.dim}`, padding: "2px 7px", fontWeight: 600 }}>ANCHOR PMID {anchor.pubmed_id}</span>
            <span style={{ ...mono(9.5, GOLD.gold), letterSpacing: "0.14em" }}>REPLYING WITHIN ANCHOR</span>
            <span style={mono(10, COOL.label)}>locked to this thread</span>
          </div>
          <div style={{ padding: 14, background: GROUND.g1, border: `1px solid ${LINE.l0}`, ...serif(15, COOL.label) }}>
            Respond to what the paper reports…
          </div>
          <span style={{ fontSize: 12, lineHeight: 1.6, color: COOL.chrome }}>
        ...
        <div style={{ padding: 18, background: GROUND.g1, border: `1px solid ${LINE.l1}`, display: "flex", flexDirection: "column", gap: 10 }}>
          <span style={{ ...mono(9.5, COOL.chrome), letterSpacing: "0.14em" }}>FLAG THIS REPLY</span>
          <span style={{ fontSize: 12, lineHeight: 1.6, color: COOL.chrome }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {FLAG_REASONS.map((r) => (
              <div key={r} style={{ display: "flex", alignItems: "center", gap: 9, opacity: 0.5 }}>
                <span style={{ width: 12, height: 12, borderRadius: 2, border: `1px solid ${COOL.chrome}`, flexShrink: 0 }} />
                <span style={{ fontSize: 12.5, color: COOL.muted }}>{r}</span>
              </div>
            ))}
          </div>
          <span style={mono(9.5, COOL.label)}>Target first action within 4h — you will be told the outcome</span>
        </div>
      </div>
    </AppLayout>
  );
