...
import { GROUND, LINE, INK, GOLD, STATE, FACE, T } from "../../lib/canonicalTokens";
import {
  getModerationQueue,
  getThread,
  type ForumPost,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: "12px 14px", background: tint, border: `1px solid ${border}` }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        <span style={{ ...mono(T.MICRO, accent), letterSpacing: "0.12em", fontWeight: 600 }}>
          {isPeer ? "PEER FLAG" : "AUTO-SIGNAL"}
        </span>
        <span style={{ ...mono(T.MICRO, accent), letterSpacing: "0.08em" }}>· {note.label}</span>
      </div>
      <span style={{ fontSize: T.META, lineHeight: 1.6, color: INK.LABEL }}>{note.body_text}</span>
      <span style={mono(T.MICRO, INK.LABEL)}>{note.disposition_text}</span>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10, padding: "16px 18px", background: GROUND.INSET, border: `1px solid ${STATE.DANGER}` }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        <span style={{ ...mono(T.MICRO, STATE.DANGER), letterSpacing: "0.12em", fontWeight: 600 }}>REMOVED BY MODERATION</span>
        <span style={mono(T.LABEL, INK.MUTE)}>{post.author_handle} · posted {post.recency_label}</span>
        <ProvenanceChip seed={post.is_seed} />
      </div>
      <span style={{ fontSize: T.META, lineHeight: 1.6, color: INK.BODY }}>
        {record?.clause_text ??
      ...
      <span style={mono(T.MICRO, INK.LABEL)}>
      ...
      <span style={mono(T.MICRO, INK.LABEL)}>Reply chain below this post preserved — thread not locked</span>
      <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
        <button
          type="button"
          onClick={() => setRevealed((v) => !v)}
          style={{ ...mono(T.MICRO, STATE.DANGER), letterSpacing: "0.1em", background: "none", border: `1px solid ${STATE.DANGER}`, padding: "4px 9px", cursor: "pointer" }}
        >
      ...
      {revealed && (
        <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: "12px 14px", background: GROUND.BASE, border: `1px dashed ${STATE.DANGER}` }}>
          <span style={{ ...mono(T.MICRO, INK.LABEL), letterSpacing: "0.12em" }}>REMOVED TEXT · VISIBLE TO REVIEWERS AND THE AUTHOR ONLY</span>
          <span style={{ fontFamily: FACE.value, fontSize: T.BODY, lineHeight: 1.65, color: INK.LABEL }}>{post.removed_body}</span>
...
function PostByline({ post, isMobile }: { post: ForumPost; isMobile: boolean }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 9, flexWrap: "wrap" }}>
      ...
      <span style={mono(T.META, INK.PRIME)}>{post.author_handle}</span>
      <VerifiedBadge small />
      <span style={mono(T.LABEL, INK.LABEL)}>{post.recency_label}</span>
...
function PostActions({ post, isMobile, onReply }: { post: ForumPost; isMobile: boolean; onReply?: (post: ForumPost) => void }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
      ...
      {onReply ? (
        <button type="button" onClick={() => onReply(post)} style={{ ...mono(T.LABEL, INK.BODY), letterSpacing: "0.08em", background: "none", border: `1px solid ${LINE.EDGE}`, borderRadius: 0, padding: "8px 12px", cursor: "pointer", ...touch }}>Reply</button>
      ) : (
...
function PostBody({ post, record, isMobile, onReply }: { post: ForumPost; record?: ModerationRecord; isMobile: boolean; onReply?: (post: ForumPost) => void }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <p style={{ margin: 0, fontFamily: FACE.value, fontSize: T.BODY, lineHeight: 1.7, color: INK.BODY, maxWidth: "72ch" }}>
        {post.body}
      </p>
  ...
  if (!loaded) {
    return (
      <AppLayout width="reading">
        <div style={{ paddingTop: 24, ...mono(T.LABEL, INK.LABEL) }}>Loading…</div>
      </AppLayout>
    );
  }
  if (!detail) {
    return (
      <AppLayout width="reading">
        <div style={{ paddingTop: 24, fontFamily: FACE.ui, color: INK.BODY }}>
          <div style={{ ...mono(T.LABEL, INK.MUTE), letterSpacing: "0.14em", marginBottom: 8 }}>THREAD NOT FOUND</div>
      ...
      <div key={post.id} style={indentStyle}>
        <div style={{ display: "flex", flexDirection: "column", gap: 10, padding: "18px 0", borderBottom: `1px solid ${LINE.HAIR}` }}>
          {nested && flat && post.compliance_state !== "removed" && parentHandle && (
            <span style={mono(T.MICRO, INK.LABEL)}>replying to {parentHandle}</span>
          )}
        ...
        {hiddenRemoved.length > 0 && (
          <div style={flat ? undefined : { marginLeft: 34, paddingLeft: 20, borderLeft: `1px solid ${LINE.HAIR}` }}>
            <div style={{ ...mono(T.LABEL, INK.LABEL), padding: "12px 0", fontStyle: "italic" }}>
    ...
    <AppLayout width="reading">
      <div style={{ fontFamily: FACE.ui, color: INK.PRIME, paddingTop: 16, display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        <div style={{ padding: isMobile ? "12px 14px" : 20, background: GROUND.RAISE, border: `1px solid ${LINE.HAIR}`, borderLeft: `2px solid ${GOLD.PRIME}`, display: "flex", flexDirection: "column", gap: isMobile ? 8 : 12, ...(isMobile ? { position: "sticky" as const, top: 0, zIndex: 10 } : {}) }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <span style={{ ...mono(T.MICRO, GOLD.PRIME), letterSpacing: "0.14em", background: "transparent", border: `1px solid ${GOLD.MUTE}`, padding: "3px 7px", fontWeight: 600 }}>PUBLICATION ANCHOR</span>
            <span style={mono(T.LABEL, INK.LABEL)}>PMID {anchor.pubmed_id}</span>
            <span style={mono(T.LABEL, INK.LABEL)}>{anchor.journal_abbrev} · {anchor.publication?.pub_year ?? "—"}</span>
            <span style={mono(T.LABEL, INK.LABEL)}>{(anchor.publication?.citation_count ?? 0)} citations</span>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 18, flexWrap: "wrap" }}>
            <a href={`https://pubmed.ncbi.nlm.nih.gov/${anchor.pubmed_id}/`} target="_blank" rel="noreferrer" style={{ ...mono(T.LABEL, GOLD.PRIME) }}>Open in PubMed ↗</a>
            <span style={mono(T.LABEL, INK.LABEL)}>Author list not reproduced in-product</span>
          </div>
          ...
          {isMobile ? (
            <span style={mono(T.MICRO, INK.LABEL)}>Scope: results as published · tap to read the paper</span>
          ) : (
            <div style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "11px 13px", background: GROUND.BASE, border: `1px solid ${LINE.HAIR}` }}>
              <span style={{ ...mono(T.MICRO, GOLD.PRIME), letterSpacing: "0.12em", flexShrink: 0, paddingTop: 2, fontWeight: 600 }}>SCOPE</span>
              <span style={{ fontSize: T.META, lineHeight: 1.6, color: INK.LABEL }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            ...
            <span style={mono(T.META, INK.PRIME)}>{thread.author_handle}</span>
            <VerifiedBadge />
            <span style={mono(T.LABEL, INK.LABEL)}>{thread.recency_label}</span>
          ...
          {thread.question_body && (
            <p style={{ margin: 0, fontFamily: FACE.value, fontSize: T.BODY, lineHeight: 1.7, color: INK.BODY, maxWidth: "70ch" }}>
              {thread.question_body}
            </p>
          )}
          <div style={{ display: "flex", gap: 14, alignItems: "center", ...mono(T.LABEL, INK.LABEL), flexWrap: "wrap" }}>
        ...
        <div style={{ marginTop: 16, padding: 18, background: GROUND.RAISE, border: `1px solid ${LINE.HAIR}`, display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <span style={{ ...mono(T.MICRO, GOLD.PRIME), letterSpacing: "0.12em", background: "transparent", border: `1px solid ${GOLD.MUTE}`, padding: "2px 7px", fontWeight: 600 }}>ANCHOR PMID {anchor.pubmed_id}</span>
            <span style={{ ...mono(T.MICRO, GOLD.PRIME), letterSpacing: "0.14em" }}>REPLYING WITHIN ANCHOR</span>
            <span style={mono(T.LABEL, INK.LABEL)}>locked to this thread</span>
          </div>
          <div style={{ padding: 14, background: GROUND.BASE, border: `1px solid ${LINE.HAIR}`, ...serif(T.BODY, INK.LABEL) }}>
            Respond to what the paper reports…
          </div>
          <span style={{ fontSize: T.META, lineHeight: 1.6, color: INK.MUTE }}>
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
            ...
            {isWriter ? (
              <button
                ...
                style={{ ...mono(T.LABEL, GOLD.PRIME), letterSpacing: "0.12em", background: "transparent", border: `1px solid ${GOLD.MUTE}`, borderRadius: 0, padding: "8px 14px", cursor: "pointer", fontWeight: 600 }}
              >
                CHECK &amp; POST REPLY
              </button>
            ) : (
        ...
        <div style={{ padding: 18, background: GROUND.BASE, border: `1px solid ${LINE.HAIR}`, display: "flex", flexDirection: "column", gap: 10 }}>
          <span style={{ ...mono(T.MICRO, INK.MUTE), letterSpacing: "0.14em" }}>FLAG THIS REPLY</span>
          <span style={{ fontSize: T.META, lineHeight: 1.6, color: INK.MUTE }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {FLAG_REASONS.map((r) => (
              <div key={r} style={{ display: "flex", alignItems: "center", gap: 9, opacity: 0.5 }}>
                ...
                <span style={{ fontSize: T.META, color: INK.LABEL }}>{r}</span>
              </div>
            ))}
          </div>
          <span style={mono(T.MICRO, INK.LABEL)}>Target first action within 4h — you will be told the outcome</span>
        </div>
      </div>
    </AppLayout>
  );
