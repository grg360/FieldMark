...
const QUEUE_META: Record<QueueState, { label: string; fg: string; bg: string; border: string }> = {
  open_peer_flag: { label: "OPEN · PEER FLAG", fg: GOLD.PRIME, bg: GOLD.WASH, border: GOLD.EDGE },
  open_auto_signal: { label: "OPEN · AUTO-SIGNAL", fg: ACTION.LINK, bg: ACTION.WASH, border: ACTION.LINK },
  blocked_at_composer: { label: "BLOCKED AT COMPOSER", fg: INK.LABEL, bg: GROUND.INSET, border: LINE.EDGE },
  resolved_removed: { label: "RESOLVED · REMOVED", fg: STATE.DANGER, bg: GROUND.INSET, border: STATE.DANGER },
};
...
function CaseDetail({ record }: { record: ModerationRecord }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14, padding: 20, background: GROUND.RAISE, border: `1px solid ${LINE.EDGE}` }}>
...
export default function ModerationPage() {
  ...
  return (
    <AppLayout width="reading">
      <div style={{ fontFamily: FACE.ui, color: INK.PRIME, paddingTop: 16, display: "flex", flexDirection: "column", gap: 20 }}>
        <PrototypeStrip />
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ display: "flex", gap: 22, ...mono(T.LABEL, INK.MUTE), flexWrap: "wrap" }}>
            ...
            <span><span style={{ color: INK.PRIME }}>4h</span> SLA</span>
          </div>
        </div>

        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "340px 1fr", gap: 16, alignItems: "start" }}>
          {/* queue list */}
          <div style={{ display: "flex", flexDirection: "column", gap: 1, background: LINE.EDGE, border: `1px solid ${LINE.EDGE}` }}>
            {records.map((r) => {
              ...
              return (
                <button
                  ...
                  style={{ textAlign: "left", cursor: "pointer", display: "flex", flexDirection: "column", gap: 8, padding: 14, background: active ? GROUND.INSET : GROUND.RAISE, border: "none", borderLeft: `2px solid ${active ? GOLD.PRIME : "transparent"}` }}
                >
