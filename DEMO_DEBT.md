...
### Co-authored paper counts on Network Influence already clickable (2026-06-26)
Logged during demo dry run as missing functionality. Verified after that MiniCollaboratorNetwork.tsx lines 116-120 already wrap the count with a navigate handler to /hcp/:hcpId/publications-with/:partnerId, with purple underline styling when hcpId is passed. False positive from dry run.
...
### Rising Voices scatter chart - hit-targets improved, edge cases remain
2026-06-26: Implemented r=8 transparent hit-target circles over visible r=5 dots in RisingVoicesChart.tsx, also disabled Scatter animation to prevent re-animation on hover. Dense middle cluster now hoverable reliably. Remaining issues: (1) yellow rising-voice dots in upper-left cluster still have inconsistent hover behavior, possibly due to overlapping hit-targets, (2) farthest-left dot at 124 followers does not register hover at all - possibly clipped by chart plot area boundary. Long-term fix: switch interaction model from hover-tooltip to click-to-pin only. This is the pattern professional data viz uses (Observable, datawrapper) and eliminates all hover edge cases. Estimated 60-90 min refactor.
