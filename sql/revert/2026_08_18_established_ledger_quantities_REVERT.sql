-- REVERT ARTIFACT for the established_ledger displayed-quantities change. 2026-08-18.
-- Branch: resurfacing
-- Covers: migrations/2026_08_18_established_ledger_quantities.sql
--
-- CAPTURED BEFORE ANYTHING WAS APPLIED, from pg_get_functiondef on the live database.
-- Verifiably pre-change: no sencit, no collab, and no join to
-- hcp_publication_leadership_v2 or hcp_network_centrality_v2.
--
-- This body ALREADY INCLUDES the EUROPE aggregate handling applied earlier today
-- (scope_bucket / scored_country), so restoring it does not unwind that -- it unwinds
-- only the two added fields.
--
-- ORDERING: revert the FRONTEND FIRST (git). The ledger columns read `sencit` and
-- `collab`; with this body restored those fields stop arriving and both columns
-- render their absent state for every row. The reverse order is safe -- the frontend
-- is what consumes them.
--
-- DATA: none. STABLE, read-only function; the two new joins read existing tables.
