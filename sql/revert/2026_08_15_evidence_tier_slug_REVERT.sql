-- REVERT ARTIFACT — hcp_nsclc_evidence_tier_v1, captured 2026-08-15 before the
-- slug repoint. This is the LIVE definition as it stood while BROKEN: it resolves
-- the therapeutic area by NAME ('NSCLC'), which stopped matching when the TA row
-- was renamed to 'Lung Cancer' earlier the same day (commit 4849c27). Restoring
-- this file re-empties the view and re-breaks community_ledger with SQLSTATE
-- 22004 — it exists to make the change reversible, not because it is correct.
-- Same discipline as sql/revert/2026_08_15_ta_name_repoint_REVERT.sql.
