-- REVERT: migrations/2026_08_31_ta_neutral_profile_rpcs.sql
-- Date: 2026-08-31. Branch: foundation-rebuild.
--
-- Restores hcp_rising_profile, hcp_profile_brief and hcp_profile_spine to the bodies that were
-- LIVE immediately before the migration (pg_get_functiondef, 2026-08-31), then drops the three
-- _ta twins. Wrappers first, drops second -- the reverse would leave a wrapper calling a
-- function that no longer exists for the width of the transaction.
--
-- NOT SAFE AFTER FRONTEND CUTOVER: once ProfileDispatch and the shells call the _ta functions
-- directly, this removes what they call. Revert the frontend first.

BEGIN;

-- 1. Restore the three original bodies, verbatim.
...
-- 2. Drop the Phase 1 profile twins.
DROP FUNCTION IF EXISTS public.hcp_rising_profile_ta(uuid, uuid);
DROP FUNCTION IF EXISTS public.hcp_profile_brief_ta(uuid, uuid);
DROP FUNCTION IF EXISTS public.hcp_profile_spine_ta(uuid, uuid);
