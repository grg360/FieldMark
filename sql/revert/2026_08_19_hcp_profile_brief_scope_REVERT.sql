-- REVERT ARTIFACT for the hcp_profile_brief scope resolution. 2026-08-19.
-- Branch: resurfacing
-- Covers: migrations/2026_08_19_hcp_profile_brief_scope.sql
--
-- CAPTURED BEFORE ANYTHING WAS APPLIED, from pg_get_functiondef on the live database.
-- Verifiably pre-change: `est` is pinned to scope_type='region' AND scope_value='US',
-- there is no scope_label anywhere, and the hcp payload has no country key.
--
-- WHAT REVERTING COSTS: every non-US profile goes back to four em-dashes in the score
-- header, and the US-only sections lose the country they gate on -- so revert the
-- FRONTEND FIRST (git), or the gates read undefined and fall through to rendering.
--
