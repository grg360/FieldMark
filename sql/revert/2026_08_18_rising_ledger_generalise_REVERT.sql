-- REVERT ARTIFACT for the rising_ledger aggregate generalisation. 2026-08-18.
-- Branch: resurfacing
-- Covers: migrations/2026_08_18_rising_ledger_generalise.sql
--
-- CAPTURED BEFORE ANYTHING WAS APPLIED, from pg_get_functiondef on the live database.
-- Verifiably pre-change: `sel` carries an is_europe boolean testing the 'EUROPE'
-- literal, and the base predicate reads the `europe` CTE directly.
--
-- WHAT REVERTING COSTS: EUROPE keeps working. An APAC selection matches no country and
-- returns an EMPTY LEDGER with no error -- 53 rows become 0 with nothing in the console.
-- If you revert this, revert the frontend in the same step so the menu stops offering
-- APAC at all; a selectable territory that silently returns nothing is worse than an
-- absent one.
--
-- ORDERING: restore this BEFORE dropping regions.aggregate_scope, which the generalised
-- body reads in its `sel` CTE.
--
