-- REVERT ARTIFACT for the established_ledger aggregate generalisation. 2026-08-18.
-- Branch: resurfacing
-- Covers: migrations/2026_08_18_established_ledger_generalise.sql
--
-- CAPTURED BEFORE ANYTHING WAS APPLIED, from pg_get_functiondef on the live database.
-- Verifiably pre-change: scored_country tests `r.scope_value = 'EUROPE'` as a literal
-- and there is no `agg` CTE.
--
-- This body already includes everything shipped earlier on 2026-08-18 -- the EUROPE
-- aggregate handling (scope_bucket / scored_country) and the displayed quantities
-- (sencit / collab). Restoring it unwinds ONLY the generalisation.
--
-- WHAT REVERTING COSTS: EUROPE keeps working (the literal it restores is EUROPE), and
-- APAC breaks in exactly the way the generalisation existed to prevent -- every row on
-- an APAC board reads its location chip as "APAC" instead of Japan, China, Korea.
--
-- ORDERING: restore this BEFORE dropping regions.aggregate_scope. This body does not
-- reference the column; the generalised one does, and would raise UndefinedColumn on
-- every ledger page load if the column went first.
--
