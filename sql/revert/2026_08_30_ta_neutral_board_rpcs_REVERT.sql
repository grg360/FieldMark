-- REVERT: migrations/2026_08_30_ta_neutral_board_rpcs.sql
-- Date: 2026-08-30. Branch: foundation-rebuild.
--
-- Restores established_ledger, rising_ledger and ledger_meta to the bodies that were LIVE
-- immediately before the Phase 1 migration (captured with pg_get_functiondef on 2026-08-30),
-- and drops the three board_* functions.
--
-- ORDER MATTERS. The wrappers are restored FIRST, then the board_* functions are dropped --
-- the reverse would leave established_ledger calling a function that no longer exists for the
-- width of the transaction. Both steps are in one transaction, so no caller ever observes the
-- gap either way; the ordering is belt and braces for anyone running these statements by hand.
--
-- SAFE TO RUN AFTER FRONTEND CUTOVER? NO. Once CohortLedger calls board_established directly,
-- this file removes the function it calls. Revert the frontend first, or drop only the parts
-- you mean to.

BEGIN;

-- ------------------------------------------------------------------------------
-- 1. Restore the three original bodies (verbatim, as they ran on 2026-08-30).
-- ------------------------------------------------------------------------------
...
DROP FUNCTION IF EXISTS public.board_meta(uuid, text);
