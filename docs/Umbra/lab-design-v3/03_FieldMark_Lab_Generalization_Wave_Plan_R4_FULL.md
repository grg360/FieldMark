...
Source text characters: 8360
Rendered characters: 10196
-->
...
# FieldMark Lab Generalization Wave Plan — Revision 4

Revision: 4
Design version: functional-lab-design.v4
Work ID: 3ec81bc4b1f347eebe16158e94e42a0c
Created: 2026-07-28T03:58:36.751096Z
Original Revision 3 artifact: `functional-lab-design-r3-artifact-de47d4c2374232df9051a111a71fdf96`
...
## Wave 0 — Final admission

Obtain exact HEAD, clean source proof and integration ownership. Admit a
complete immutable schema-only artifact. Discover/hash `sql/README.md` and the
24-script `TA_GENERALIZATION_INVENTORY`. Build the command manifest and approve
the bounded v2 patch design.
The approximately 210 repository-wide Python scripts are discovery context,
not the execution set; only an admitted subset of the authoritative 24-script
Breast surface may run.

## Wave 1 — Foundation and Tier 1

After `LAB_FOUNDATION_AND_V2_PATCH_AUTHORIZED`, create only the managed
worktree, isolated local PostgreSQL, configuration, admitted bootstrap, local
processes and Tier 1 fixtures. Implement/test the v2 safeguard. Foundation
egress remains disabled. Replay bootstrap twice from clean databases and fail
as `FAILED_BOOTSTRAP_VALIDATION` on any missing object or unexpected row.

## Waves 2–4 — Configuration and bounded generalization

Retain unvalidated Breast configuration/domain proposals, admitted backend/SQL
work and inventory-scoped frontend generalization. These do not authorize a
corpus run.

## Wave 5 — Separately authorized corpus validation

Requires `BREAST_CORPUS_RUN_AUTHORIZED`, explicit domains/caps/retention,
proposal-validation authority and admitted v2 commands. It is not included in
the Foundation/v2 authorization package.

## Wave 6 — Functional review

Require rerun evidence, NSCLC regression, Breast functional validation,
vocabulary evidence, development review and founder review.

## Follow-on Wave 7 — Container parity

Reproduce the accepted functional build in the separately authorized OCI
adapter. The cross-TA rehearsal remains the stronger production-like surface.

After Foundation acceptance, Breast waves should be proposed and governed
conversationally through UmbraOS rather than manually shepherded tickets.
...
**Original artifact id:** functional-lab-design-r3-artifact-de47d4c2374232df9051a111a71fdf96
