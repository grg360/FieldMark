...
## ADDENDUM — 2026-05-28 (Day 3 changes)

These supersede/correct entries above where they conflict:

### Narrative script name
- The live narrative generator is **`generate_narratives_v2.py`** (32KB, the one we run). `claude_layer.py` listed above is the older/v1-era layer. Use `generate_narratives_v2.py --cohort {rising_star|established|community|all} --target-version v2`. **`--target-version v2` is REQUIRED** (default is v1 → writes to wrong table). Flags: `--cohort`, `--community-top` (default 500), `--dry-run`, `--force`. NO `--limit` or single-HCP flag (for a test run: start a cohort, Ctrl+C after a few, inspect).

### OpenAlex author-metrics enrichment script
- **`openalex_author_enrichment.py`** — the script run this session to populate `hcp_author_metrics_v2` (citations, h-index, counts_by_year). Distinct from `openalex_pipeline.py`. Flags: `--workers N` (safe ceiling 8, use 4), `--resume` (existence-based skip), `--snapshot-date YYYY-MM-DD` (added today, for resuming into a prior snapshot), `--dry-run`, `--limit`, `--offset`. Corpus now COMPLETE: 229,238 distinct HCPs at snapshot 2026-05-27.
  - Two bugs fixed today: bounded in-flight dispatch window (was hanging on upfront submit of all futures >5k); dedup in upsert_batch by (hcp_id, snapshot_date) keeping highest works_count.
  - DO NOT chunk this via offset (the old `enrich_chunks.ps1` approach) — offset paging over the `hcps_v2!inner` join is unstable and caused a 30K gap. Use `--resume` instead.

### scoring_pipeline.py
- Now reads **`career_first_pub_year_v2`** (the corrected column), NOT `career_first_pub_year`. Writes by default; `--dry-run` opts out; `--target-version v2` REQUIRED.

### career_first_pub_year_v2 (NEW column on hcps_v2)
- Corrected career-start year from a publication-density heuristic over `counts_by_year`. Backfilled this session via SQL (not a standalone script — the CTE is in HANDOVER/chat history). If re-run needed after future enrichment, the UPDATE is in the Day 3 chat transcript. `career_age_years` (generated column) now derives from this.

### community_scoring.py — note
- DOES write the populated signal columns (patient_volume, pharma_engagement, group_practice_signal, career_years, publication_signal). The catalog's "needs rebuild" tech-debt note is partially stale — the script works; the geographic-gate reframe is the open piece. Does NOT depend on career_first_pub_year, so community does NOT need re-scoring after the career-year fix.
