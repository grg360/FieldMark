/* ==== 20. TERRITORY INCLUDES INSTITUTION-PLACED HCPs BY DEFAULT ====
   REVERSES THE DEFAULT SET IN 03. Same two functions, same signatures, same
   bodies; only the default on p_include_institution_placed changes, false to
   true. CREATE OR REPLACE is correct here -- a parameter default is not part
   of a function's identity, so nothing is dropped and no grant is lost.

   WHY. Measured on the live CRC established ledger after the clear:

     TERRITORY = UNITED STATES    3,561
     TERRITORY = SOUTHEAST           94   (was 226 before the clear)

   132 people left the Southeast. Not edge cases: Bassel El-Rayes and Christina
   Wu at Emory, Wei Zheng at Vanderbilt, Qian Shi at Mayo Florida, Richard Kim
   at Moffitt, Michael Lee at UNC. They are in the Southeast. Their state came
   from their institution rather than from NPPES, which is a statement about
   evidence, not about geography.

   The original reasoning was: do not place someone in a territory on a state
   we cannot verify. But the chip already carries that -- 'GA . INSTITUTION'
   tells the MSL exactly what basis they are looking at. Excluding the row
   instead produces a territory that quietly drops 58% of its members with
   nothing on screen to say so.

   That is a nameless absence, and this platform does not ship those. A
   qualified presence the user can judge beats a silent removal they cannot
   see. 'Absence is never zero' applies to the territory filter too.

   The flag stays. An MSL who wants NPPES-verified placement only can still
   pass false, and board_meta still reports include_institution_placed so the
   surface can say which mode produced the list. */

DROP FUNCTION IF EXISTS public.board_established(uuid, integer, integer, text[], text[]);

CREATE OR REPLACE FUNCTION public.board_established(p_ta_id uuid, p_limit integer DEFAULT 1000, p_after_rank integer DEFAULT 0, p_states text[] DEFAULT '{}'::text[], p_countries text[] DEFAULT '{US}'::text[], p_include_institution_placed boolean DEFAULT true)
 RETURNS json
 LANGUAGE plpgsql
...
CREATE OR REPLACE FUNCTION public.board_rising(p_ta_id uuid, p_limit integer DEFAULT 1000, p_after_rank integer DEFAULT 0, p_states text[] DEFAULT '{}'::text[], p_countries text[] DEFAULT '{US}'::text[], p_include_institution_placed boolean DEFAULT true)
 RETURNS json
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
