...
| **Feed** `/` (`FeedLayout`, defined in **App.tsx** — the audit originally attributed it to `DashboardTabs.tsx`, which was a stale unrouted copy, deleted 2026-08-05) | NavBar + PeopleNavStrip (Newsreader serif register) + DOLHeroPanel | GlobalFooter | none — card grid | `.fm-screen` maxWidth **880**, `system-ui` | `useMediaQuery` (767) |
...
By font (`system-ui` override) and palette, still **live**: `ProfileScreen` (/me/settings), `WatchlistsPage` + TrackedHcpsList family, `FollowUpsPage` family, `FieldInsightsScreen` + the FieldInsights composer/cards (`#4a90e2`/`#5a9b7f`/`#9b6dff`/`#3fb8af`), `LandscapeRoute` + QuadrantChart/Leaderboard (the rising board's only data-bearing view — being folded into the rising surface design, do not retire first), `WelcomeWizard`, `SignupScreen`, `ScoringExplainedModal`, the DOL modals, `FieldIntelligenceShared` (toasts/modals), `ActionTray`, `FilterDrawer`, `HCPCard`, `SnoozePicker`, `AddToWatchlistPopover`, `RelationshipSection/*`. (Corrections 2026-08-05: `SearchScreen` and `FilterButton` were listed live here in error — the reachability trace showed both already orphaned pre-sweep; deleted along with `LandscapeScreen`, which became dead when the feedOverlay branch was removed. See §5.16 addendum.) Also **dead but still in-tree** at audit time and dominating the raw counts: `DetailScreen` and its subtree (see §5.16). The 275-use `#6b6a65` census in §1 is effectively a map of this generation, live and dead together.
